...
function createBird() {
  return {
    ...
    vy: 0,
    invincible: 0,
    size: window.GAME_CONFIG.birdSize,
  };
}
...
function drawBird(b) {
  ...
  if (b.invincible > 0 && Math.floor(b.invincible / 3) % 2 === 0) {
    ctx.restore();
    return;
  }
  ...
  ctx.fillRect(-s / 2, s / 4 + Math.sin(game.frame * 0.3) * 3, s * 0.7, s * 0.3);
  ctx.fillStyle = '#fff';
  ctx.fillRect(s / 4, -s / 4, s / 4, s / 4);
  ctx.fillStyle = '#000';
  ctx.fillRect(s / 4 + 2, -s / 4 + 2, s / 8, s / 8);
  ctx.fillStyle = '#ff8800';
  ctx.fillRect(s / 2, -s / 8, s / 4, s / 8);
...
function drawPipe(pipe) {
  ...
  const dark = shadeColor(pc, -50);
  const W = pipe.w;

  // Bottom pipe
  ctx.fillStyle = pc;
  ctx.fillRect(pipe.x, pipe.gapBottom, W, canvas.height - pipe.gapBottom);
  ctx.fillStyle = dark;
  ctx.fillRect(pipe.x, pipe.gapBottom, 6, canvas.height - pipe.gapBottom);
  ctx.fillStyle = shadeColor(pc, 30);
  ctx.fillRect(pipe.x + W - 6, pipe.gapBottom, 6, canvas.height - pipe.gapBottom);
  ctx.fillStyle = pc;
  ctx.fillRect(pipe.x - 4, pipe.gapBottom, W + 8, 16);
  ctx.fillStyle = dark;
  ctx.fillRect(pipe.x - 4, pipe.gapBottom, 6, 16);

  // Top pipe
  ctx.fillStyle = pc;
  ctx.fillRect(pipe.x, 0, W, pipe.gapTop);
  ctx.fillStyle = dark;
  ctx.fillRect(pipe.x, 0, 6, pipe.gapTop);
  ctx.fillStyle = shadeColor(pc, 30);
  ctx.fillRect(pipe.x + W - 6, 0, 6, pipe.gapTop);
  ctx.fillStyle = pc;
  ctx.fillRect(pipe.x - 4, pipe.gapTop - 16, W + 8, 16);
  ctx.fillStyle = dark;
  ctx.fillRect(pipe.x - 4, pipe.gapTop - 16, 6, 16);
}
...
function drawHUD() {
  ...
  for (let i = 0; i < game.lives; i++) {
    ...
    ctx.fillRect(canvas.width - 16 - i * 20, 10, 12, 12);
  }
}
...
function handleCollision() {
  const b = game.bird;
  const action = window.GAME_CONFIG.collisionAction;
...
export function gameLoop() {
  ...
  if (game.frame % cfg.pipeInterval === 0) {
    const minTop = 60;
    const maxTop = canvas.height - cfg.pipeGap - 60;
